...
if __name__ == "__main__":
    ...
    # make a ros node
    rospy.init_node("publish_trajectories")
    ...
    traj_setpoint_pubs = {
        ns: rospy.Publisher(f"/{ns}/offboard/setpoint", PoseStamped, queue_size=10)
        for ns in ROS_NAMESPACES
    }

    # loop over the times and publish the setpoints
    rate = rospy.Rate(1 / traj_update_interval)
    curr_traj_idx = 0
    while not rospy.is_shutdown():
        for ns, traj in trajs.items():
            pose = traj[curr_traj_idx]
            setpoint = PoseStamped(
                header=Header(stamp=rospy.Time.now(), frame_id="map"), pose=pose
            )
